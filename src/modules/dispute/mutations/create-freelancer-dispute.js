...
const {
  PAID_KEY, COMPLETED, EVENT_RAISE_DISPUTE_VALIDITY_IN_HOURS, WEDLANCER_ASSURED, SUCCESS,
...
const createFreelancerDispute = async (_, args, ctx) => {
  try {
    const {
      models: {
    ...
    const recruiterProfile = await UserProfileModel.findOne({ where: { userId }, attributes: ['typeKey'] });
    const { typeKey: recruiterTypeKey } = recruiterProfile;
    if (recruiterTypeKey !== PAID_KEY) {
    ...
    const freelancerProfile = await UserProfileModel.findOne({ where: { userId: freelancerId } });
    const { typeKey: freelancerTypeKey } = freelancerProfile;
    if (freelancerTypeKey !== WEDLANCER_ASSURED) {
      throw new CustomApolloError(getMessage('ISSUE_RAISE_IS_ALLOWED_ONLY_FOR_WEDLANCER_ASSURED_FREELANCER'), localeService);
    }

    const existingDispute = await DisputeModel.findOne({ where: { userId: freelancerId, raisedBy: userId, eventId } });
    if (existingDispute) {
    ...
    const disputeData = {
      eventId, userId: freelancerId, message, ticketNo, raisedBy: userId,
    };
    ...
    disputeLogger(`Error while creating freelancer dispute from web : ${error}`, ctx, 'error');
    throw error;
  }
};

module.exports = createFreelancerDispute;
