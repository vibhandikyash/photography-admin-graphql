...
const createNotificationForDisputeRaisedAgainstFreelancer = require('../../../../../shared-lib/notifications/disputes/create-notification-for-dispute-raised-against-freelancer');
...
const createRecruiterEventIssue = async (req, res, next) => {
  try {
    const { user, localeService } = req;
    const { eventId, freelancerId, message } = req.body;
    ...
    // Only PAID user are allowed to raised issues.
    ...
    // Issue only raised to those user have assigned to this event
    ...
    if (!freelancer) throw new ApiError('INVALID_FREELANCER', BAD_REQUEST);

    const freelancerProfile = await UserProfileModel.findOne({
      where: {
        userId: freelancerId,
      },
    });
    if (freelancerProfile.typeKey !== 'WEDLANCER_ASSURED') throw new ApiError('ISSUE_RAISE_NOT_ALLOWED', BAD_REQUEST);

    // Recruiter is only raised issue once upon a user
    ...
    if (dispute) {
      ...
      createNotificationForDisputeRaisedAgainstFreelancer(user.id, freelancerId, dispute, localeService);
      createNotificationForDisputeRaisedToAdmin(dispute.id, localeService);
    }
    return sendSuccessResponse(res, 'SUCCESS', OK, response);
  } catch (error) {
    eventLogger(`Error from create-event-issue: ${error.message}`, null, 'error');
    return next(error);
  }
};

module.exports = createRecruiterEventIssue;
